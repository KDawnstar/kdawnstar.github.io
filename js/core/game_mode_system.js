// [카시야스 보스전] 게임 체험 모드 / 가이드 UI 전용 레이어
...
const GameModeSystem = {
    ...
    controls: [
        ...
        { group: '스킬 / 기능', key: '1', name: '회복 물약', desc: '최대 HP의 30%를 회복합니다. 각 진행 단계마다 최대 8회 사용할 수 있으며, 재사용 대기시간은 10초입니다.' },
    ...
    bindStartButtons(gameState) {
        document.querySelectorAll('[data-game-mode]').forEach(btn => {
            if (btn.dataset.boundGameMode === '1') return;
            btn.dataset.boundGameMode = '1';
            btn.addEventListener('click', () => this.startMode(btn.getAttribute('data-game-mode'), gameState));
        });
    },

    bindClearButton(gameState) {
        const btn = document.getElementById('gameClearReturnBtn');
        if (!btn || btn.dataset.boundGameClear === '1') return;
        btn.dataset.boundGameClear = '1';
        btn.addEventListener('click', () => this.returnToTitle(gameState));
    },

    showGameClear(gameState) {
        if (!gameState || (gameState.bossPractice && gameState.bossPractice.enabled)) return false;
        gameState.gameClearActive = true;
        gameState.presentationStarted = false;
        gameState.keys = {};
        this.toggleHelp(false);
        const start = document.getElementById('gameModeStartScreen');
        if (start) start.classList.add('hidden');
        const clear = document.getElementById('gameClearScreen');
        if (clear) {
            clear.classList.add('visible');
            clear.setAttribute('aria-hidden', 'false');
        }
        return true;
    },
    ...
    returnToTitle(gameState) {
        ...
        // 타이틀 복귀용 재로딩이 이전 모드(특히 GUIDE)의 보정값을 이어받지 않도록
        // 모드 상태를 먼저 해제한 뒤 전투 런타임을 초기화한다.
        if (gameState) {
        ...
        if (typeof window.resetBossBattleToTitle === 'function') {
            window.resetBossBattleToTitle();
        }
        this.toggleHelp(false);
        this.applyBodyMode(null);
        const start = document.getElementById('gameModeStartScreen');
    ...
    startMode(mode, gameState) {
        ...
        if (next === this.modes.GUIDE) this.applyInitialCooldownPolicyToActiveBosses(gameState);
        if (typeof lastTime !== 'undefined') lastTime = performance.now();
        const screen = document.getElementById('gameModeStartScreen');
    ...
    adjustInitialPatternCooldown(gameState, pattern, cooldown) {
        const base = Math.max(0, parseFloat(cooldown) || 0);
        if (!this.isGuide(gameState) || !pattern) return base;
        ...
        if (category !== 'MAJOR' || condType !== 'COOLDOWN_READY') return base;
        if (['231008', '231009', '232008', '232009'].includes(patternId)) return base;
        return 15;
    },

    adjustGroggyTime(gameState, groggyTime) {
        const base = Math.max(0.2, parseFloat(groggyTime) || 0.2);
        if (!this.isGuide(gameState)) return base;
        if (base >= 14) return 10;
        if (base >= 9) return 6;
        return base;
    },

    applyInitialCooldownPolicyToActiveBosses(gameState) {
        if (!gameState || !Array.isArray(gameState.monsters)) return;
        for (const monster of gameState.monsters) {
            const boss = monster && monster.boss;
            if (!boss) continue;
            ...
            for (const pattern of patterns) {
                ...
                const normalInitial = Math.max(0, parseFloat(pattern.Pattern_Initial_Cooltime) || 0);
                const lateInitial = parseFloat(pattern.Late_Phase_Pattern_Initial_Cooltime);
                const useLateInitial = !!boss.isLatePhase && Number.isFinite(lateInitial) && lateInitial > 0;
                const base = useLateInitial ? Math.max(0, lateInitial) : normalInitial;
                boss.patternCooldowns[patternId] = this.adjustInitialPatternCooldown(gameState, pattern, base);
            }
        }
    },
    ...
    renderControlGuide(container) {
        ...
        container.innerHTML = groups.map(group => {
            ...
            return `<div class="control-guide-card${group === '개발 / 특수 기능' ? ' special' : ''}">
                <div class="control-guide-title">${group}</div>
                ${items.map(item => {
                    const displayKey = item.key === '방향키 더블 탭' ? '방향키 ×2' : item.key;
                    const keyClass = group === '개발 / 특수 기능' ? ' debug' : (item.key === 'D' ? ' guard' : '');
                    return `<div class="control-row guide-compact-row"><span class="keycap${keyClass}">${displayKey}</span><span class="guide-control-name">${this.escapeHtml(item.name)}</span></div>`;
                }).join('')}
            </div>`;
        }).join('');
    },
    ...
    renderHelpContent() {
        const content = document.getElementById('gameModeHelpContent');
        if (!content) return;
        ...
        content.innerHTML = groups.map(group => {
            ...
            if (!items.length) return '';
            return `<section class="game-mode-help-group"><h3>${group}</h3>${items.map(item => `<div class="game-mode-help-row"><span class="game-mode-help-key">${item.key}</span><div><b>${item.name}</b><p>${this.escapeHtml(item.desc).replace(/\n/g, '<br>')}</p></div></div>`).join('')}</section>`;
        }).join('');
    },
    ...
    prepareHudSlots() {
        ...
        Object.entries(roleByMask).forEach(([maskId, role]) => {
            ...
            let cooldownNumber = slot.querySelector('.skill-cooldown-number');
            if (!cooldownNumber) {
                cooldownNumber = document.createElement('div');
                cooldownNumber.className = 'skill-cooldown-number';
                cooldownNumber.setAttribute('aria-hidden', 'true');
